...
<body>
  ...
  <div class="topbar">
    <div class="profile-area">
      <button class="btn btn-add-member" onclick="location.href='/main_user/user_reg_index.do'">
        + Add Member
      </button>
    </div>
  </div>
  ...
  <div class="main-content">
    <!-- 검색 폼 -->
    <form action="/main_user/doRetrieve.do" method="get" class="row g-2 mb-3 align-items-center">
      ...
      <div class="col-auto">
        <select name="searchDiv" id="searchDiv" class="form-select form-select-sm">
          ...
          <option value="40" <c:if test="${40 == search.searchDiv}">selected</c:if>>이메일</option>
        </select>
      </div>
      <div class="col-auto">
        <input type="search" name="searchWord" id="searchWord" class="form-control form-control-sm" placeholder="검색어 입력" value="${search.searchWord}">
      </div>
      <div class="col-auto">
    ...
    <div class="card">
              ...
              <tr title="더블클릭 하면 상세 정보를 볼 수 있습니다.">
                ...
                <td class="text-center">
                  <c:choose>
                    <c:when test="${vo.isAdmin == 'Y'}">관리자</c:when>
                    <c:otherwise>
                      <a href="${pageContext.request.contextPath}/main_user/setAdmin.do?userId=${vo.userId}" class="btn btn-outline-secondary btn-sm">관리자 임명</a>
                    </c:otherwise>
                  </c:choose>
                </td>
                <td class="text-center">
                  <a href="/main_user/doSelectOne.do?userId=${vo.userId}" class="btn btn-outline-secondary btn-sm">수정</a>
                </td>
                <td class="text-center">
                  <button class="btn btn-outline-danger btn-sm" onclick="deleteUser('${vo.userId}')">삭제</button>
                </td>
              </tr>
            </c:forEach>
          </tbody>
      ...
      <!-- 페이징 -->
      <c:set var="totalPages" value="${(totalCnt + search.pageSize - 1) / search.pageSize}" />
      <c:if test="${totalPages lt 1}">
        <c:set var="totalPages" value="1" />
      </c:if>
      <div class="pagination-container">
        <c:if test="${search.pageNo > 1}">
          <a href="doRetrieve.do?pageNo=${search.pageNo - 1}&pageSize=${search.pageSize}&searchDiv=${search.searchDiv}&searchWord=${search.searchWord}" class="pagination-btn">이전</a>
        </c:if>
        <c:forEach var="i" begin="1" end="${totalPages}">
          <c:choose>
            <c:when test="${i == search.pageNo}">
              <span class="pagination-btn disabled" style="background-color: #28a745; color: #fff;">${i}</span>
            </c:when>
            <c:otherwise>
              <a href="doRetrieve.do?pageNo=${i}&pageSize=${search.pageSize}&searchDiv=${search.searchDiv}&searchWord=${search.searchWord}" class="pagination-btn">${i}</a>
            </c:otherwise>
          </c:choose>
        </c:forEach>
        <c:if test="${search.pageNo < totalPages}">
          <a href="doRetrieve.do?pageNo=${search.pageNo + 1}&pageSize=${search.pageSize}&searchDiv=${search.searchDiv}&searchWord=${search.searchWord}" class="pagination-btn">다음</a>
        </c:if>
      </div>
    </div>
  </div>
