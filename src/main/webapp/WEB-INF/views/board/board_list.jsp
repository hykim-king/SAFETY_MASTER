...
<head>
    ...
    <!-- 제목 설정 -->
	<c:choose>
		<c:when test="${board_div eq '10'}"><title>공지사항</title></c:when>
		<c:when test="${board_div eq '20'}"><title>자유게시판</title></c:when>
		<c:when test="${board_div eq '30'}"><title>통계자료실</title></c:when>
	</c:choose>
    ...
    <div id="container" class="container">
        ...
        <main class="container my-4">
            <article class="col-md-12">
                <!-- 제목 영역 -->
                <c:choose>
                    <c:when test="${board_div eq '10'}"><h2 class="text-center mb-4">공지사항</h2></c:when>
                    <c:when test="${board_div eq '20'}"><h2 class="text-center mb-4">자유게시판</h2></c:when>
                    <c:when test="${board_div eq '30'}"><h2 class="text-center mb-4">통계자료실</h2></c:when>
                </c:choose>
                <hr>
                
                <!-- 검색 조건 폼 -->
                <form class="row g-3 mb-3 align-items-center justify-content-end" action="#" name="userForm" id="userForm" method="get">
                    ...
                    <div class="col-auto">
                        <input class="form-control" type="search" name="searchWord" id="searchWord" value="${search.searchWord}">
                    </div>
                    <div class="col-auto">
                    ...
                    <div class="col-auto">
                        <input class="btn btn-primary" type="button" value="조회" id="doRetrieveBtn">
                        
                        <!-- 등록 버튼 조건: 자유게시판(20)은 로그인한 회원, 공지사항(10)과 통계자료실(30)은 관리자만 -->
                        <c:if test="${ (board_div eq '20' and not empty sessionScope.user)
                            or ((board_div eq '10' or board_div eq '30') and not empty sessionScope.user and sessionScope.user.isAdmin eq 'Y') }">
                            <input class="btn btn-success" type="button" value="등록" id="moveToRegBtn">
                        </c:if>
                    </div>
                </form>
				    ...
				    <table id="listTable" class="table table-bordered table-hover table-sm">
				        ...
				        <tbody>
				            <c:choose>
				                    ...
				                    <c:forEach var="vo" items="${boardList}">
				                        <tr class="table-hover">
				                            ...
				                            <td class="text-left py-2 px-3">
				                                <a href="/board/doSelectOne.do?div=${board_div}&boardId=${vo.boardId}" class="text-decoration-none text-dark">
				                ...
				                <c:otherwise>
				                    <tr>
				                        <td colspan="5" class="text-center py-2 px-3">데이터가 존재하지 않습니다.</td>
				                    </tr>
				                </c:otherwise>
				            </c:choose>
				        </tbody>
